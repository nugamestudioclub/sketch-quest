...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        (Vector2 center, Vector2 size) = GetGroundCheck(playerCollider);
        ...
        Gizmos.DrawWireSphere(bombEndPos, groundCheckHeight);
        ...
        {
            Gizmos.DrawWireSphere(GameLogic.GetPlatformOffset(Position, degrees, PlatformDistance), groundCheckHeight);
        }
    }
    ...
    private (Vector2 center, Vector2 size) GetGroundCheck(Collider2D col2d)
    {
        Bounds bounds = col2d.bounds;

        Vector2 size = col2d is BoxCollider2D box 
            ? new(bounds.size.x + box.edgeRadius*1.9f, box.edgeRadius + groundCheckHeight)
            : new(bounds.size.x, groundCheckHeight);

        return (new(bounds.center.x,bounds.min.y) , size);
    }
    private void HandleGrounded()
    {
        ...
        (Vector2 center, Vector2 size) = GetGroundCheck(playerCollider);
        isGrounded = Physics2D.OverlapBox(center, size, angle: 0, ~LayerMask.GetMask("Player", "Switch"));
        if (isGrounded)
        {
            canDoubleJump = true;
            canAirDash = true;
